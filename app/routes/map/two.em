...
class MapTwoRoute extends Ember.Route with ServerTalk
	model:->
		@getServer("users", {targets_with_locations: true}, "json"
		).then (response) =>
			controller = @controllerFor('map.two')
			numberOfTargets = response.users.length
			locations_array = response.users[1].locations.map (loc) -> {
